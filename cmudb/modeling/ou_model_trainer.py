...
class OUModelTrainer:
    ...
    def train_specific_model(self, data, y_transformer_idx, method_idx):
        ...
        y_transformers = [
            None,
            data_transforming_util.OPUNIT_Y_TRANSFORMER_MAP[data.opunit],
        ]
        x_transformer = data_transforming_util.OPUNIT_X_TRANSFORMER_MAP[data.opunit]
        regressor = model.Model(
            methods[method_idx],
            y_transformer=y_transformers[y_transformer_idx],
    ...
    def train_data(self, data, summary_file):
        ...
        for i, y_transformer in enumerate(y_transformers):
            for m, method in enumerate(methods):
                ...
                regressor = model.Model(
                    method, y_transformer=y_transformer, x_transformer=x_transformer
                )
                ...
                for j, d in enumerate(evaluate_data):
                    ...
                    if (
                        j == 1
                        and eval_error < min_percentage_error
                        and y_transformer == y_transformers[-1]
                        and (
                            data.opunit not in data_info.instance.ARITHMETIC_OPUNITS
                            or method == "lr"
                        )
                    ):
                    ...
                    if j == 1:
                        io_util.write_csv_result(
                            summary_file,
                            data.opunit.name,
                            [label] + list(percentage_error),
                        )
    ...
    def train(self):
        ...
        header = ["OpUnit", "Method"] + [
            target.name for target in data_info.instance.OU_MODEL_TARGET_LIST
        ]
        ...
        for filename in sorted(glob.glob(os.path.join(self.input_path, "*.csv"))):
            print(filename)
            data_list = opunit_data.get_ou_runner_data(
                filename,
...
if __name__ == "__main__":
    ...
    aparser.add_argument(
        "--test_ratio", type=float, default=0.2, help="Test data split ratio"
    )
    aparser.add_argument(
        "--trim",
        default=0.2,
    ...
    trainer = OUModelTrainer(
        args.input_path,
        args.model_results_path,
        args.ml_models,
        args.test_ratio,
        args.trim,
        args.expose_all,
        args.txn_sample_rate,
    )
